              ...
              return (
                <Link key={zone.id} href={`/plants/${pid}/zones/${zone.id}`}>
                  <div className="bg-card border border-card-border rounded-xl p-5 hover:border-primary/40 cursor-pointer group transition-all">
                    ...
                    {/* Mini sparkline */}
                    <div className="mt-3 h-8 -mx-1 opacity-50 group-hover:opacity-90 transition-opacity">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={zone.sparkline} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                          <defs>
                            <linearGradient id={`zg-${zone.id}`} x1="0" y1="0" x2="0" y2="1">
                              <stop offset="5%" stopColor={color} stopOpacity={0.25} />
                              <stop offset="95%" stopColor={color} stopOpacity={0} />
                            </linearGradient>
                          </defs>
                          <Area type="monotone" dataKey="v" stroke={color} strokeWidth={1.5} fill={`url(#zg-${zone.id})`} dot={false} isAnimationActive={false} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
