...
import {
  ShieldCheck, Mail, Loader2, CheckCircle2, RotateCcw,
...
function EmailStep({ onSent }: {
  ...
  async function send(e: React.FormEvent) {
    ...
    try {
      ...
      // Safe JSON parse — server may return HTML on 404 if API is outdated
      ...
      try { b = JSON.parse(text); } catch {
        setError(r.status === 404
          ? "API route not found — the server needs to be updated."
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Enter your whitelisted email to receive a one-time password.
      </p>
    ...
    <div className="bg-white rounded-2xl shadow-xl p-10 w-full max-w-md flex flex-col items-center text-center">
      ...
      {smtpOn ? (
        ...
        <>
          ...
          <p className="text-sm text-gray-500">
            SMTP is off — email not sent to{" "}
            <span className="font-semibold text-gray-800">{masked}</span>
          </p>
          <p className="text-xs text-amber-600 mt-1">Get the code from server logs ↓</p>
        </>
      )}
  ...
  async function resend() {
    ...
    try {
      ...
      const b = await r.json() as {
        ok?: boolean; expiresInMs?: number; resendCooldownMs?: number;
        message?: string; mailerEnabled?: boolean;
      };
      if (r.ok) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-amber-800 mb-1">
                SMTP off — OTP not emailed
              </p>
              <p className="text-xs text-amber-700 mb-1.5">Run this on the server to get your code:</p>
      ...
      <p className="text-sm text-gray-500 mb-5">
        {smtpOn ? "Check your email for the 6-digit code." : "Find the 6-digit code in your server logs."}
      </p>
